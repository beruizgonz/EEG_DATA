...
dataset_path = 'data/deap_stress_4s_subject.h5'

dataset = StressDataset(dataset_path,normalize='normalization')
dm = ERPDataModule(dataset)

ckpt = os.path.join('checkpoints', 'SSL-1s-val_loss=0.15-epoch=98.ckpt')
...
encoder_args = {'feat_dim': 16, 'seq_len':512, 'd_model':64, 'n_heads': 8, 'num_layers': 2, 'dim_feedforward': 256, 'dropout': 0.2, 'activation': 'gelu', 'norm': 'LayerNorm'}
encoder1 = TSTransformerEncoder(**encoder_args)
conv1 = Conv1DNet()
model_erp = EEG_ERP(learning_rate = 0.001, convnet = conv, encoder = encoder, decoder = ERP_decoder(d_model = 64, ts_steps=512))
...
checkpoint_callback = ModelCheckpoint(
    ...
    filename='ckpt-{epoch:02d}-{train_f1:.2f}-4s',
    save_top_k=1,
    mode='max',
)
