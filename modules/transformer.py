...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src, mask = None, src_key_padding_mask = None):
        ...
        """
        output = src
        attn_output_list = [] # Store outputs from each layer
        if src.is_cuda:
            attn_output = torch.zeros((src.shape[1], src.shape[0], src.shape[0]), device = 'cuda')
        else:
            attn_output = torch.zeros((src.shape[1], src.shape[0], src.shape[0]), device = 'cpu') # batch, seq_len, seq_len
        
        for mod in self.layers:
            output, attn = mod(output, src_mask = mask, src_key_padding_mask = src_key_padding_mask)
            attn_output_list.append(attn)    
        if self.norm is not None:
            output = self.norm(output)
        return output, attn_output_list


# https://github.com/gzerveas/mvts_transformer/blob/master/src/models/ts_transformer.py
# Modified to also return attention 
